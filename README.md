...
class TaiDataSynch(object):
    ...
    def get_user_response(self, eon_id):
        ...
        try:
            ...
            user_response.raise_for_status()
            user_data = user_response.json().get('data', [])
            return list(set(user['user.user'] for user in user_data)) if user_data else []
        except requests.exceptions.RequestException as e:
            print(f"Error fetching users for EON ID {eon_id}: {e}")
            return []
    ...
    def synch_tai_data(self):
        try:
            ...
            # Prepare for update and insert operations
            values_to_update = []
            values_to_insert = []

            # Check which EON IDs exist in both the TAI data and the database
            common_eon_ids = eonid_in_tai.intersection(eonid_db)

            for record in data:
                eon_id = record["system.eon_id"]
                users = self.get_user_response(eon_id)
                technology_owner = list(set([record["system.primary_technology_owner"]] + 
                                            str(record.get("system.technology_owner", "")).split(",")))
                business_owner = list(set([record["system.primary_business_owner"]] + 
                                          str(record.get("system.business_owner", "")).split(",")))
...
class TaiDataSynch(object):
    ...
    async def get_user_response(self, eon_id, session):
        user_url = f"{self.base_url}/{self.user_dataset}/?f={self.filter}={eon_id}&c={self.user_column}"
        try:
            async with session.get(user_url, auth=HTTPKerberosAuth(principal=""), timeout=60) as response:
                response.raise_for_status()
    ...
    async def fetch_all_user_data(self, eon_ids):
        async with aiohttp.ClientSession() as session:
            tasks = [self.get_user_response(eon_id, session) for eon_id in eon_ids]
    ...
    def synch_tai_data(self):
        try:
            ...
            eon_ids_to_fetch = list(eonid_in_tai)
            users_data = asyncio.run(self.fetch_all_user_data(eon_ids_to_fetch))
